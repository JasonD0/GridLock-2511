...
public class Menu extends JPanel {
	...
	public Menu(GridLockFrame game) {
		...
		box.add(Box.createRigidArea(new Dimension(0,212))); // add gap

		// create start button
		start.setAlignmentX(CENTER_ALIGNMENT);
		start.setBorderPainted(false);
		start.setFocusable(false);
		Image menuIcon = new ImageIcon("button_play.png").getImage();
		...
		Image miniGameIcon = new ImageIcon(miniGamePng).getImage();
		miniGameIcon = miniGameIcon.getScaledInstance(116, 67, Image.SCALE_SMOOTH);
		...
		box.add(Box.createRigidArea(new Dimension(0,5))); // add gap
